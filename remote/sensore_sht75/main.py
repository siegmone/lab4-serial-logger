...
def unpack(data):
    ...
    packets_raw = np.array([[p[:4], p[4:]] for p in packets])
    hums = packets_raw[:, 0]
    temps = packets_raw[:, 1]
    return hums, temps
...
def convert_temp(temp_hex):
    d1, d2 = -39.7, 0.01
    temp = [hex2dec(i)*d2+d1 for i in temp_hex if i]
    return temp


def convert_rh(rh_hex):
    c1, c2, c3 = -2.04681, 0.0367, -1.5955*10**-6
    rh = [hex2dec(i)**2*c3+c1+c2*hex2dec(i) for i in rh_hex if i]
    return rh
...
reconstructed = open(
    f"data/sht75_Hum_Temp_RUN_{fn_dt}_{acqHours}_h_RECONSTRUCTED.txt", "w")
reconstructed.write(
    "Ntrigger Anno Mese Giorno SecondiInGiorno HUM_RAW HUM_REC TEMP_RAW TEMP_REC")
