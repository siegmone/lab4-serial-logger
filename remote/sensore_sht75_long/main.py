...
def unpack(data):
    ...
    packets_raw = np.array(
        [[p[:4], p[4:]] for p in packets if p[:4] and p[4:]]
    )
    hums = packets_raw[:, 0]
    temps = packets_raw[:, 1]
    return hums, temps
...
def data_acquisition(ser, tmax, sleep, checkpoint):
    ...
    while True:
        ...
        dt = datetime.now()
        dt_formatted = dt.strftime("%Y-%m-%d_%H:%M:%S")
...
print(f"Starting Communication for {acqSecs} seconds")
try:
